  ...
  async play(guildId, song, voiceChannel, textChannel) {
    try {
      if (!this.lavalink || !this.lavalink.shoukaku) {
        return textChannel.send('⏳ 오디오 엔진 초기화 중입니다. 5초 뒤에 다시 시도해 주세요. (v3.1.6)');
      }
      ...
      console.log(`[v3.1.6] Audio attempt for guild ${guildId}. Found ${nodesArray.length} potential nodes, Ready: ${readyNodes.length}`);

      if (readyNodes.length === 0) {
        let msg = `🛰️ 오디오 서버 연결 대기 중... (현재 연결된 서버: ${readyNodes.length}/${nodesArray.length}) (v3.1.6)`;
      ...
      if (!player) {
        ...
        console.log(`[v3.1.6] Created new Lavalink player for guild ${guildId}`);
      }
      ...
      console.error('[v3.1.6] Play Error:', e);
      textChannel.send('❌ 재생 중 오류가 발생했습니다. (Lavalink 노드 서버 확인 요망)');
    }
  }

  async join(voiceChannel, textChannel) {
    console.log(`[v3.1.6] Preparing Lavalink join for ${voiceChannel.name}`);
    return true;
  }
}
