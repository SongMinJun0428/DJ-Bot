  ...
  async play(guildId, song, voiceChannel, textChannel) {
    try {
      if (!this.lavalink || !this.lavalink.shoukaku) {
        return textChannel.send('⏳ 오디오 엔진 초기화 중입니다. 10초 뒤에 다시 시도해 주세요. (v4.1.2)');
      }
      ...
      console.log(`[v4.1.2] Audio attempt for guild ${guildId}. Found ${nodesArray.length} potential nodes, Ready: ${readyNodes.length}`);

      if (readyNodes.length === 0) {
        let msg = `🛰️ 오디오 서버 연결 대기 중... (현재 연결된 서버: ${readyNodes.length}/${nodesArray.length}) (v4.1.2)`;
