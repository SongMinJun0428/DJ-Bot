    ...
    try {
      ...
      if (query.startsWith('http')) {
        ...
        if (!fromChannel && interaction.deferred) {
            const content = playResult && playResult.status === 'WAITING' ? playResult.message : `✅ **${song.title}** 처리가 시작되었습니다. (v3.1.4)`;
            await interaction.editReply({ content }).catch(() => {});
        }
      } else {
        console.log(`[v3.1.4] Searching for: ${query}`);
        ...
        try {
          ...
          console.error('[v3.1.4] Selection timeout or error:', e);
          interaction.editReply({ content: '선택 시간이 초과되었습니다.', components: [] }).catch(() => {});
        }
      }
    } catch (e) {
      console.error('[v3.1.4] Play command Error:', e);
