# 🎧 음악으로 즐기는 프리미엄 DJ 봇 가이드 (v4.0.8)

이 문서는 봇의 설치, 설정 및 사용 방법을 한국어로 자세히 설명합니다. (v4.0.1 정식 버전)
...
- 1.  **/설정 (`/setup`)**: 서버에 `DJ 봇 음악 채널`이라는 이름의 음악 전용 채널을 생성합니다.
    ...
    - **파일 업로드**: 음악 채널에 MP3 파일을 그냥 올리는 것만으로도 즉시 재생이 가능합니다. (v4.0.1 신기능)
    - 채널의 메시지 입력란에 노래 제목을 그냥 입력하는 것만으로 검색 및 재생이 가능합니다.

## 3. 주요 명령어 및 기능

*   **/play [제목/링크/파일]**: 음악을 검색하거나 링크, 파일로 재생합니다.
*   **/skip**: 현재 곡을 건너뜁니다.
*   **/queue**: 현재 대기열을 확인합니다.
...
## 5. 기술 요구 사항 및 호스팅 (v4.0.1)

### 필수 환경
*   **Node.js**: **v22.12.0 이상** (디스코드 보안 패치 DAVE E2EE 대응 필수 버전)
*   **패키지**: `discord.js v14.17.3+`, `shoukaku v4.x`, `kazagumo v3.x`
*   **오디오 엔진**: Lavalink v4 서버 연동 (코딩 내에 최적화된 공용 노드 포함)

### ⚠️ 중요: 2026년 음성 연결 문제 해결
디스코드의 최신 보안 정책인 **DAVE (종단간 암호화)**로 인해 발생하는 소리 끊김 및 시그널링 멈춤 문제를 **v4.0.1** 정식 버전에서 완벽히 해결했습니다.
...
4.  `Settings` -> `Variables`에서 아래 항목을 반드시 추가합니다.
    *   `NIXPACKS_NODE_VERSION`: `22` (최신 보안 환경 구성)
    *   `DISCORD_TOKEN`: 본인의 봇 토큰
5.  **데이터 저장**: 플레이리스트 데이터를 유지하려면 Railway 설정에서 `/db` 폴더에 **Volume**을 연결하세요.
6.  배포 성공 후 로그에 `v2.8.0` 문구가 보이면 정상입니다.
